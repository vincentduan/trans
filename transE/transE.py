...
def data_loader(file):
    ...
    with codecs.open(file1, 'r') as f:
        ...
        for line in content:
            ...
            triple_list.append([h_, t_, r_])  # 所有的三元组集合

            entity_set.add(h_)  # 所有的entity集合, 包含h和t
            entity_set.add(t_)

            relation_set.add(r_)  # 所有的relation集合,
...
class TransE:
    ...
    def emb_initialize(self):  # 初始化entity和relation的值
        relation_dict = {}
        entity_dict = {}
    ...
    def train(self, epochs):
        nbatches = 400
        batch_size = len(self.triple_list) // nbatches   # 1207
        ...
        for epoch in range(epochs):
            ...
            for k in range(nbatches):
                # Sbatch:list
                Sbatch = random.sample(self.triple_list, batch_size)  # 从triple_list中随机获取batch_size个元素，作为一个list返回
                Tbatch = []
